...
export default function Dashboard() {
  ...
  const fetchUserReports = async () => {
    try {
      ...
      // Check content existence and get names
      const validReports = [];
      const contentNames = {};
      
      for (const report of userReportsData) {
        // Get content names for display (only for events and groups)
        if (report.contentType === 'event' || report.contentType === 'group') {
          ...
          // For other content types, just include them (existence checked in ReportController)
          validReports.push(report);
        }
      }
  ...
  const fetchData = async () => {
    try {
      ...
      if (!userCountry) {
        // If no address, show content sorted by popularity with a note
        ...
        const sortedPosts = [...allPosts]
          .filter(post => post.userId !== userData.id) // Exclude current user's posts
          .sort((a, b) => b.friendCount - a.friendCount)
          .slice(0, 6);
        
        // Show popular users (most friends) who are accepting guests
        ...
        // Original logic for when user has an address
        ...
        // Add user data to posts and exclude current user's posts
        ...
        // Filter and match users - sort by friends count first, then country match
        const matchedUsers = users
          ...
          .sort((a, b) => b.friendCount - a.friendCount); // More friends first
        
        setMatchingUsers(matchedUsers);
        
        // Sort posts - first by location match, then by likes, then by friends count
        const sortedPosts = postsWithUserCountry.sort((a, b) => {
          ...
          // If same location match, sort by likes
          const likesDiff = (b.likedBy?.length || 0) - (a.likedBy?.length || 0);
          if (likesDiff !== 0) return likesDiff;
          
          // If same likes, sort by friends count
          return (b.friendCount || 0) - (a.friendCount || 0);
        });
        setPosts(sortedPosts);
      }

      // Fetch posts from followed users (works the same regardless of location)
  ...
  const handleAddReply = async (postId, commentId, replyText) => {
    try {
      ...
      const fakeReplyId = uuidv4(); // generate random ID (safe until real sync)
        ...
        prevPosts.map(post => {
          ...
          const updatedComments = post.comments.map(comment => {
            ...
            const updatedReplies = {
              ...comment.replies,
              [fakeReplyId]: {
                replyId: fakeReplyId, // Important to set
                reply: replyText,
                likes: [],
                userId: userData.id,
                user: userData,
      ...
      setPosts(prevPosts =>
        prevPosts.map(post => {
          ...
          const updatedComments = post.comments.map(comment => {
            ...
            if (replyId) {
              // If replyId is provided, remove reply inside this comment
              const updatedReplies = { ...comment.replies };
              delete updatedReplies[replyId];
  
              return { ...comment, replies: updatedReplies };
            } else {
              // If no replyId => means deleting whole comment
              return null;
            }
